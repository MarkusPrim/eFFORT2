...
class BtoV:
    ...
    def __init__(
        ...
        self.N0 = 6 * self.mB * self.mV ** 2 / 8 / (4*np.pi) ** 4 * self.GF ** 2 * self.eta_EW ** 2 * self.BR_Dstar_decay
        # Helper variable to work with angular coefficients
        self.xi = self.GF ** 2 * self.Vcb ** 2 * self.mB ** 3 / 2 / np.pi ** 4 * self.eta_EW ** 2 * self.BR_Dstar_decay 
