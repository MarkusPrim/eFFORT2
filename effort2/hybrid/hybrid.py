...
class Hybrid:
    ...
    def __init__(self, hybrid_config: Union[AnyStr, PathLike] = None) -> None:
        ...
        Args:
            hybrid_config (Union[AnyStr, PathLike], optional): Path to json file defining the binning of the Hybrid model. Has to contain the six attributes:
    ...
    def calculate_weight(self, x: numpy.array, hybrid_weights: numpy.histogramdd) -> float:
        ...
        Args:
            x (numpy.array): Phase space location of the inclusive prediction in the form (El_B, q2, mX).
            hybrid_weights (numpy.histogramdd): Hybrid weights generated by :func:`~hybrid.Hybrid.generate_hybrid_weights`.

        Returns:
            float: The hybrid weight w_i at the given phase space point x.
        """
    ...
    def generate_hybrid_weights(self, inclusive: pandas.DataFrame, exclusive: pandas.DataFrame) -> numpy.histogramdd:
        ...
        The given names are defaults from hybrid_binning.json, they can be changed in the json file if required.

        Nota bene:
            * It is required to use pure generator level MC, without any selection cuts applied.

        Args:
            inclusive (pandas.DataFrame): Inclusive MC.
            exclusive (pandas.DataFrame): Exclusive MC. Can contain any number of resonances.

        Returns:
            numpy.histogramdd: 3D histogram containing the weights of the Hybrid model. To be used with
